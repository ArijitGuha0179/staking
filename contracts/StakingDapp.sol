...
contract StakingDapp is Ownable,ReentrancyGuard{
    ...
    function deposit(uint256 _pid,uint _amount)public nonReentrant{
        require(amount>0,"Amount must be greater than 0");
        PoolInfo storage pool=poolInfo[_pid];
        UserInfo storage user=userInfo[_pid][msg.sender];

        // if the user has previously deposited any amount we make sure that the rewardpending is sent to the user
        
        if(user.amount>0){
            uint pending=_calcPendingReward(user,_pid);
            pool.rewardToken.transfer(msg.sender,pending);

            _createNotification(_pid,pending,msg.sender,"Pending Reward Claim");
        }
    ...
    function withdraw(uint256 _pid,uint _amount)public nonReentrant{
        PoolInfo storage pool=poolInfo[_pid];
        UserInfo storage user=userInfo[_pid][msg.sender];

        require(user.amount>=_amount,"Insufficient Balance");
        require(user.lockUntil<=block.timestamp,"Reward Locked");
    ...
    function pendingReward(uint256 _pid,address _user)public view returns(uint256){
        return _calcPendingReward(userInfo[_pid][_user],_pid);
    }
